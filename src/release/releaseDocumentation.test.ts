...
describe('release documentation and workflow', () => {
  ...
  it('keeps the release workflow publish allowlist explicit', () => {
    ...
    expect(releaseWorkflow).toContain('packages/cantonjs-splice-interfaces publish')
    expect(releaseWorkflow).toContain('packages/cantonjs-splice-scan publish')
    expect(releaseWorkflow).toContain('packages/cantonjs-splice-validator publish')
    expect(releaseWorkflow).toContain('packages/cantonjs-splice-token-standard publish')
    expect(releaseWorkflow).toContain('packages/cantonjs-wallet-adapters publish')
  })
})
