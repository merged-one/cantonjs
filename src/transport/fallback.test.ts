...
describe('fallback', () => {
  ...
  it('falls back on ConnectionError', async () => {
    const primary = mockTransport('http://primary', vi.fn().mockRejectedValue(
      new ConnectionError('http://primary'),
    ))
  ...
  it('does not retry on HttpError (application-level)', async () => {
    ...
    await expect(transport.request({ method: 'GET', path: '/test' }))
  ...
  it('throws the last error when all transports fail', async () => {
    ...
    const secondary = mockTransport('http://secondary', vi.fn().mockRejectedValue(
      new ConnectionError('http://secondary'),
    ))
    const transport = fallback({ transports: [primary, secondary] })
  ...
  it('tries all transports in order', async () => {
    const order: number[] = []
    const t1 = mockTransport('http://t1', vi.fn().mockImplementation(async () => {
      order.push(1)
      throw new ConnectionError('http://t1')
    }))
    const t2 = mockTransport('http://t2', vi.fn().mockImplementation(async () => {
      order.push(2)
      throw new ConnectionError('http://t2')
    }))
    const t3 = mockTransport('http://t3', vi.fn().mockImplementation(async () => {
      order.push(3)
      return { success: true }
    }))
